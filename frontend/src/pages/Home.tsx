...
function BookingForm({ onCreated, testIdPrefix }: BookingFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-10" data-testid={testId("booking-form")}>
      <section className="space-y-5" data-testid={testId("care-section")}><div className="flex items-center gap-3"><span className="flex size-7 items-center justify-center rounded-full bg-[#0E776C] text-xs font-bold text-white">1</span><div><h4 className="font-serif text-xl text-[#164D59]" data-testid={testId("care-heading")}>Choose your care</h4><p className="text-xs text-[#5C6479]" data-testid={testId("care-copy")}>Select the closest fit—we can refine it together.</p></div></div><fieldset className="space-y-3"><legend className="text-sm font-semibold text-[#2B2D42]">Consultation type</legend><div className="grid gap-2 sm:grid-cols-3">{consultationTypes.map((type) => <button key={type.value} type="button" aria-pressed={form.consultation_type === type.value} onClick={() => setForm((current) => ({ ...current, consultation_type: type.value }))} className={`min-h-16 rounded-xl border px-3 py-3 text-left text-sm transition-all duration-300 hover:-translate-y-0.5 ${form.consultation_type === type.value ? "border-[#0E776C] bg-[#EAF2F1] text-[#164D59] shadow-sm" : "border-[#E5DEC9] bg-white text-[#5C6479] hover:border-[#839788]"}`} data-testid={testId(`consultation-type-${type.value}`)}><span className="block font-semibold">{type.label}</span><span className="mt-1 block text-xs opacity-75">{type.detail}</span></button>)}</div></fieldset><div className="space-y-2"><Label htmlFor={`${testIdPrefix}-focus-area`}>Health focus</Label><select id={`${testIdPrefix}-focus-area`} required value={form.focus_area} onChange={updateField("focus_area")} className="flex h-12 w-full rounded-lg border border-[#E5DEC9] bg-white px-3 text-sm text-[#2B2D42] outline-none ring-offset-white transition focus:border-[#0E776C] focus:ring-2 focus:ring-[#0E776C]/20" data-testid={testId("focus-select")}>{focusAreas.map((area) => <option key={area.id} value={area.id}>{area.title}</option>)}</select></div></section>

      <section className="space-y-5 border-t border-[#E5DEC9] pt-8" data-testid={testId("schedule-section")}><div className="flex items-center justify-between gap-4"><div className="flex items-center gap-3"><span className="flex size-7 items-center justify-center rounded-full bg-[#0E776C] text-xs font-bold text-white">2</span><div><h4 className="font-serif text-xl text-[#164D59]" data-testid={testId("schedule-heading")}>Pick a preferred time</h4><p className="text-xs text-[#5C6479]" data-testid={testId("schedule-copy")}>Available Tuesday, Thursday and Saturday.</p></div></div><span className="hidden rounded-full bg-[#F4F1E8] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.12em] text-[#0E776C] sm:inline-flex" data-testid={testId("timezone-note")}>India time</span></div>{bookingOptions.data && <div className="grid grid-cols-3 gap-2 sm:grid-cols-6" data-testid={testId("date-options")}>{bookingOptions.data.available_dates.slice(0, 6).map((option) => <button key={option.date} type="button" aria-pressed={form.preferred_date === option.date} onClick={() => { setForm((current) => ({ ...current, preferred_date: option.date })); setDateError(""); }} className={`min-h-16 rounded-xl border p-2 text-center transition-all duration-300 ${form.preferred_date === option.date ? "border-[#0E776C] bg-[#0E776C] text-white shadow-sm" : "border-[#E5DEC9] bg-white text-[#35504D] hover:border-[#0E776C]"}`} data-testid={testId(`date-option-${option.date}`)}><span className="block text-[10px] font-bold uppercase tracking-[0.12em] opacity-70">{option.day_label.slice(0, 3)}</span><span className="mt-1 block font-serif text-lg">{option.display_label}</span></button>)}</div>}{bookingOptions.isError && <p className="rounded-lg bg-[#F4F1E8] p-3 text-xs text-[#5C6479]" data-testid={testId("date-options-fallback")}>Date suggestions are unavailable. Enter your preferred date below.</p>}<div className="grid gap-5 sm:grid-cols-[0.8fr_1.2fr]"><div className="space-y-2"><Label htmlFor={`${testIdPrefix}-preferred-date`}>Preferred date</Label><Input id={`${testIdPrefix}-preferred-date`} type="date" required value={form.preferred_date} onChange={updateField("preferred_date")} data-testid={testId("date-input")} /></div><fieldset className="space-y-2"><legend className="text-sm font-semibold text-[#2B2D42]">Preferred time</legend><div className="flex flex-wrap gap-2">{["Morning preference", "Afternoon preference", "Evening preference"].map((time) => <button key={time} type="button" aria-pressed={form.preferred_time === time} onClick={() => setForm((current) => ({ ...current, preferred_time: time }))} className={`min-h-11 rounded-full border px-4 text-sm transition-all duration-300 ${form.preferred_time === time ? "border-[#0E776C] bg-[#0E776C] text-white" : "border-[#E5DEC9] bg-white text-[#5C6479] hover:border-[#0E776C]"}`} data-testid={testId(`time-${time.split(" ")[0].toLowerCase()}`)}>{time.replace(" preference", "")}</button>)}</div></fieldset></div>{dateError && <p className="text-sm font-medium text-[#B84F3A]" role="alert" data-testid={testId("date-error")}>{dateError}</p>}</section>

      <section className="space-y-5 border-t border-[#E5DEC9] pt-8" data-testid={testId("details-section")}><div className="flex items-center gap-3"><span className="flex size-7 items-center justify-center rounded-full bg-[#0E776C] text-xs font-bold text-white">3</span><div><h4 className="font-serif text-xl text-[#164D59]" data-testid={testId("details-heading")}>Your contact details</h4><p className="text-xs text-[#5C6479]" data-testid={testId("details-copy")}>We use these only to confirm your consultation.</p></div></div><div className="grid gap-5 sm:grid-cols-2"><div className="space-y-2"><Label htmlFor={`${testIdPrefix}-full-name`}>Your name</Label><Input id={`${testIdPrefix}-full-name`} required value={form.full_name} onChange={updateField("full_name")} placeholder="Full name" data-testid={testId("name-input")} /></div><div className="space-y-2"><Label htmlFor={`${testIdPrefix}-phone`}>Phone number</Label><Input id={`${testIdPrefix}-phone`} required value={form.phone} onChange={updateField("phone")} placeholder="+91 ..." data-testid={testId("phone-input")} /></div></div><div className="space-y-2"><Label htmlFor={`${testIdPrefix}-email`}>Email address</Label><Input id={`${testIdPrefix}-email`} type="email" required value={form.email} onChange={updateField("email")} placeholder="you@example.com" data-testid={testId("email-input")} /></div><div className="space-y-2"><Label htmlFor={`${testIdPrefix}-notes`}>What would you like to discuss? <span className="font-normal text-[#5C6479]">(optional)</span></Label><Textarea id={`${testIdPrefix}-notes`} value={form.notes} onChange={updateField("notes")} placeholder="A few words are enough..." className="min-h-24 resize-none" data-testid={testId("notes-input")} /></div></section>

      {errorMessage && <p className="text-sm font-medium text-[#B84F3A]" role="alert" data-testid={testId("form-error")}>{errorMessage}</p>}
      <Button type="submit" disabled={mutation.isPending} className="h-13 w-full rounded-full bg-[#E07A5F] text-white shadow-[0_10px_24px_rgba(224,122,95,0.22)] hover:bg-[#c9654c]" data-testid={testId("submit-button")}>{mutation.isPending ? "Saving your request..." : "Request my consultation"}{!mutation.isPending && <ArrowRight className="ml-2 size-4" />}</Button>
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-white text-[#263D3A] selection:bg-[#E4B45E]/30">
      ...
      <header className="absolute inset-x-0 top-10 z-40 border-0 bg-transparent px-4 py-3 sm:px-6 lg:px-10" data-testid="site-header">
        <div className="mx-auto flex h-[4.5rem] max-w-7xl items-center justify-between rounded-sm bg-white px-5 shadow-[0_12px_35px_rgba(23,60,58,0.12)] sm:px-8 lg:px-6">
          ...
          <nav className="hidden items-center gap-7 lg:flex" aria-label="Main navigation" data-testid="desktop-navigation">
            {["Care pathways", "About Dr. Nisha", "Book consultation", "Questions"].map((item, index) => <a key={item} href={`#${["care", "about", "booking", "faq"][index]}`} className="text-[13px] font-semibold text-[#35504D] transition-colors duration-300 hover:text-[#0E776C]" data-testid={`nav-${item.toLowerCase().replaceAll(" ", "-")}-link`}>{item}</a>)}
          </nav>
      ...
      <main id="top">
        ...
        <section id="booking" className="scroll-mt-20 bg-[#F4F1E8] px-5 py-24 sm:px-8 lg:px-12 lg:py-32" data-testid="booking-section">
          <div className="mx-auto max-w-6xl">
            <Reveal className="grid gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-end">
              <div><p className="mb-5 text-xs font-bold uppercase tracking-[0.22em] text-[#E07A5F]" data-testid="booking-eyebrow">A simple request</p><h2 className="font-sans text-4xl font-medium leading-tight tracking-[-0.035em] text-[#164D59] sm:text-6xl" data-testid="booking-heading">Book one thoughtful conversation.</h2></div>
              <div><p className="max-w-xl text-lg leading-relaxed text-[#52706B]" data-testid="booking-intro">Choose your care, preferred date, and contact details in one place. Dr. Nisha's team will personally confirm the final time.</p><div className="mt-5 flex flex-wrap gap-x-6 gap-y-3 text-sm"><a href="tel:+916355734167" className="flex items-center gap-2 font-semibold text-[#0E776C] hover:text-[#E07A5F]" data-testid="booking-phone-link"><Phone className="size-4" /> +91 63557 34167</a><a href="mailto:nishaghelani78@gmail.com" className="flex items-center gap-2 font-semibold text-[#0E776C] hover:text-[#E07A5F]" data-testid="booking-email-link"><Mail className="size-4" /> nishaghelani78@gmail.com</a></div></div>
            </Reveal>
            <Reveal className="mx-auto mt-12 max-w-4xl rounded-[1.5rem] border border-[#D9DFD4] bg-white p-6 shadow-[0_24px_70px_rgba(22,77,89,0.09)] sm:p-10 lg:p-12" delay={0.1}>
              <div className="mb-10 flex flex-col gap-5 border-b border-[#E5DEC9] pb-7 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="font-serif text-3xl text-[#164D59]" data-testid="booking-form-heading">Request a consultation</h3><p className="mt-2 text-sm text-[#5C6479]" data-testid="booking-form-subheading">No payment now · reply within one working day</p></div><div className="flex items-center gap-2" id="availability" data-testid="availability-section"><CalendarDays className="mr-1 size-5 text-[#E07A5F]" />{["Tue", "Thu", "Sat"].map((day) => <span key={day} className="rounded-full bg-[#EAF2F1] px-3 py-1.5 text-xs font-bold text-[#0E776C]">{day}</span>)}</div></div>
